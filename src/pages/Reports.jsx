...
import { getBudgetUsage, getExpenseByAllocation, getMonthTransactions, sumByType } from '../utils/calculations.js';
...
export default function Reports() {
  ...
  const allocationData = getExpenseByAllocation(monthTransactions, budgets);
  const chartData = allocationData.slice(0, 5).map((item) => ({ name: item.name, total: item.amount }));
  ...
  const criticalBudget = monthBudgets
    .map((budget) => ({ budget, usage: getBudgetUsage(budget, monthTransactions) }))
  ...
  return (
    <div className="page playful-reports-page">
      <header className="header playful-page-header">
        ...
        <button className="icon-btn playful-icon-btn" type="button" aria-label="Export laporan"><Download size={18} /></button>
      </header>

      <section className="playful-report-hero">
        <div className="row-between">
          <div><span>Periode laporan</span><strong>{monthLabel(month, year)}</strong></div>
          <LineChart size={22} />
        </div>
      ...
      <Card className="playful-chart-card">
        <div className="section-head">
          <div><p className="section-kicker">Grafik</p><h2>Pengeluaran per Alokasi</h2></div>
      ...
      <section className="playful-summary-grid reports-summary-grid">
        <div className="playful-stat-card income"><span>Pemasukan</span><strong>{formatRupiah(income)}</strong></div>
        <div className="playful-stat-card expense"><span>Pengeluaran</span><strong>{formatRupiah(expense)}</strong></div>
        <div className="playful-stat-card saving"><span>Saving rate</span><strong>{savingRate}%</strong></div>
      </section>

      <Card className="playful-section-card">
        <div className="section-head"><div><p className="section-kicker">Detail Alokasi</p><h2>Komposisi pengeluaran</h2></div></div>
        {allocationData.length === 0 ? <p className="muted tiny">Belum ada pengeluaran bulan ini.</p> : allocationData.map((item) => {
          const pct = expense ? Math.round((item.amount / expense) * 100) : 0;
          return (
            <div className="budget-row playful-budget-row" key={item.budgetId}>
      ...
      <Card className="playful-insight-card">
        ...
        <div className="playful-insight-item">
          ...
          <div><h2>{top ? `${top.name} paling dominan` : 'Belum ada insight'}</h2><p>{top ? `Pengeluaran terbesar bulan ini adalah ${top.name} sebesar ${formatRupiah(top.amount)}.` : 'Input transaksi terlebih dahulu agar sistem dapat menampilkan insight bulanan.'}</p></div>
        </div>
        <div className="playful-insight-item">
          <span><PiggyBank size={17} /></span>
          <div><h2>Saving rate {savingRate}%</h2><p>{income ? `Keluarga menyisihkan sekitar ${savingRate}% dari pemasukan bulan ini.` : 'Tambahkan pemasukan untuk menghitung saving rate.'}</p></div>
        </div>
        {criticalBudget && (
          <div className="playful-insight-item warning">
            <span><AlertTriangle size={17} /></span>
            <div><h2>{criticalBudget.budget?.name || 'Alokasi'} perlu diperhatikan</h2><p>Penggunaan sudah mencapai {criticalBudget.usage.percentage}% dengan sisa {formatRupiah(criticalBudget.usage.remaining)}.</p></div>
          </div>
        )}
      </Card>
    </div>
